...
bot.dialog('teaminfo', [
    ...
    function (session,results,next) {
        ...
        if(session.conversationData.stage == 3)
        {
            ...
            var jsonContent = session.conversationData.teamInfoData[key];
            console.log('key : %s ; jsonContent : %s' , key, jsonContent);
            
            /*
            [
                builder.CardAction.messageBack(session, 'postback.teaminfo.'+key+'.TAAlias', 'TA Contact'),
                builder.CardAction.messageBack(session, 'postback.teaminfo.'+key+'.CSSWiki', 'Team Wiki'),
                builder.CardAction.messageBack(session, 'postback.teaminfo.'+key+'.SupportTopicTier1', 'Support Topics'),
                builder.CardAction.messageBack(session, 'postback.teaminfo.'+key+'.TeamAlias', 'Team Alias'),
                builder.CardAction.messageBack(session, 'postback.teaminfo.'+key+'.PODs', 'Pods')
                //.text("TechLeads : \n" + jsonContent["TAAlias"] + " \n Products: " + jsonContent["SupportTopicTier1"])             
            ]
            */
            ...
            new builder.HeroCard(session)
                ...
                .buttons(
                    [
                        builder.CardAction.messageBack(session, '', 'TA Contact')
                            .text('postback.teaminfo.'+key+'.TAAlias'), 
                        builder.CardAction.messageBack(session, '', 'Team Wiki')
                            .text('postback.teaminfo.'+key+'.CSSWiki'),
                        builder.CardAction.messageBack(session, '', 'Support Topics')
                            .text('postback.teaminfo.'+key+'.SupportTopicTier1'),
                        builder.CardAction.messageBack(session, '', 'Team Alias')
                            .text('postback.teaminfo.'+key+'.TeamAlias'),
                        builder.CardAction.messageBack(session, '', 'Pods')
                            .text('postback.teaminfo.'+key+'.PODs')
                    ]
                )
            ]);
